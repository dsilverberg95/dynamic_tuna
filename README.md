...
The $m$ configurations at which $A$ is evaluated are selected via Sobol sampling (https://en.wikipedia.org/wiki/Sobol_sequence), and the computations of $A$ are vectorized and generally very efficient. Setting $m \leq 10000000$ is generally safe, unless the surrogate model is an extremely large/deep random forest, in which case a smaller number should be chosen based on the user's computational/time budget.
