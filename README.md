...
The $m$ configurations at which $A$ is evaluated are selected via Sobol sampling (https://en.wikipedia.org/wiki/Sobol_sequence), and the computations of $A(\theta)$ at each sampled $\theta$ are vectorized (i.e. fast, except in the case where the surrogate is a large random forest).
